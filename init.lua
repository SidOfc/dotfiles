...
-- general {{{
  ...
  vim.opt.wildignore    = vim.opt.wildignore + {'.git', '.DS_Store', 'node_modules'}
  ...
  if vim.fn.has('persistent_undo') == 1 then
    local persistent_undo_directory = vim.fn.expand('~/.config/vim-persisted-undo')
  ...
  function close_buffer()
    if #vim.fn.getbufinfo({buflisted = 1, windows = {vim.fn.bufwinid('%')}}) > 1 then
      vim.cmd('bdelete')
...
-- statusline {{{
  ...
  function status_line()
    local group     = status_mode_groups[vim.fn.mode():lower()] or status_mode_groups['n']
...
-- fzf {{{
  ...
  noremap('n', '<leader>I', ':call v:lua.fzf_mkdx_headers()<Cr>', {silent = true})
...
-- nvim-treesitter {{{
  if not bootstrap_plugins then
    require('nvim-treesitter.configs').setup({
      ensure_installed = {
        'c', 'cpp', 'css', 'fish', 'html', 'java', 'javascript', 'json',
        'lua', 'perl', 'php', 'pug', 'python', 'ruby', 'rust', 'scss',
